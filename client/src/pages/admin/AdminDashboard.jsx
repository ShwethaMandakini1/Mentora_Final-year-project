 ...
 * 
 * PURPOSE:
 * Admin dashboard focused on USER MANAGEMENT and PLATFORM OVERSIGHT
 * Shows: user counts, subscriptions, quick actions
 * Does NOT show: submission stats (those are lecturer-specific)
 * 
 * STATS SHOWN:
 * - Total Users, Students, Lecturers, New This Week
 * - Subscription breakdown (Free/Pro/Institution)
 * - Quick actions: Manage Users, Manage Admins, Subscriptions, Broadcast
 */
...
const SvgIcon = ({ name, size = 18 }) => {
  const icons = {
    ...
    megaphone: <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M3 11l18-5v12L3 13v-2z"/><path d="M11.6 16.8a3 3 0 1 1-5.8-1.6"/></svg>,
...
export default function AdminDashboard() {
  ...
  // Returns cached data if less than 10 minutes old, otherwise fetches fresh
  const fetchStats = async () => {
    ...
    try {
      ...
      // If 401, token expired → redirect to signin
  ...
  // ── Data for stat cards ──────────────────────────────────────────────────
  // USER STATS: Total, Students, Lecturers, New This Week
  const userCards = [
  ...
  // SUBSCRIPTION BREAKDOWN: Free / Pro / Institution counts
  ...
  // QUICK ACTION BUTTONS: Navigate to key admin pages
  const quickActions = [
    ...
    { label: 'Broadcast Message',     icon: 'megaphone', route: '/admin/broadcast' },
  ];
  ...
  return (
    <AdminLayout>
      ...
      <div className="adm-content" style={{ paddingTop: 0 }}>
        ...
        {/* ── BOTTOM GRID: Subscription breakdown + Quick actions ───────── */}
              ...
              {quickActions.map(a => (
                <button
                  ...
                  style={{
                    ...
                    textAlign: 'left', fontSize: 13, width: '100%', justifyContent: 'flex-start',
                  }}
                >
        ...
        {/* ── BOTTOM ROW: Platform-wide admin count ───────────────────────── */}
        <div className="adm-stats-grid adm-stats-grid-1">
          <div className="adm-stat-card adm-stat-ocean">
            <div className="adm-stat-icon"><SvgIcon name="shield" /></div>
            <div className="adm-stat-num">{stats.users.admins}</div>
