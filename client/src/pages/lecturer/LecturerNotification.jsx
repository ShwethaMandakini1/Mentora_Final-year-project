...
const Icons = {
  ...
  Document:    () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="18" height="18"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" /></svg>,
...
// ─────────────────────────────────────────────────────────────────────────────
// Type config — only types a lecturer will ever receive
// ─────────────────────────────────────────────────────────────────────────────
const TYPE_CONFIG = {
  // Student submitted for pre-approval review
  approval_requested: {
  ...
  // Student asked for a regrade
  regrade_requested: {
    icon:  <Icons.Sync />,
    color: 'rgba(234,88,12,0.10)',
    text:  '#ea580c',
    label: 'Re-grade Request',
  },
...
// ─────────────────────────────────────────────────────────────────────────────
// Filters — no "Deadlines" for lecturers, added "Re-grade" and "Admin"
// ─────────────────────────────────────────────────────────────────────────────
const FILTERS = ['All', 'Unread', 'Review Requests', 'Re-grade', 'Admin'];

const FILTER_MAP = {
  'All':             () => true,
  'Unread':          n => !n.read,
  'Review Requests': n => n.type === 'approval_requested',
  'Re-grade':        n => n.type === 'regrade_requested',
  'Admin':           n => n.type === 'admin_message',
};
...
// Which notification types have a navigable destination on the lecturer side
const isViewable = (n) =>
  n.type === 'approval_requested' || n.type === 'regrade_requested';
...
export default function LecturerNotifications() {
  ...
  const fetchNotifications = useCallback(async (silent = false) => {
    try {
      ...
      // Filter: keep only types the lecturer should see
      const all  = res.data.notifications || [];
      const data = all.filter(n =>
        ['approval_requested', 'regrade_requested', 'admin_message'].includes(n.type)
      );
  ...
  const handleView = async (n) => {
    ...
    if (n.type === 'regrade_requested')  navigate('/lecturer/submissions');
  };
      ...
      <div className="ios-topbar">
        ...
        {notifications.length > 0 && (
          <div className="ios-topbar-right" style={{ gap: '12px', display: 'flex' }}>
            ...
            <button
              className="ios-topbar-btn ios-topbar-btn--danger"
              onClick={deleteAll}
              style={{ display: 'flex', alignItems: 'center', gap: '6px' }}
            >
                        ...
                        >
                          ...
                          <div className="ios-notif-body">
                            ...
                            <div className={`ios-notif-title${!n.read ? ' ios-notif-title--unread' : ''}`}>
                              {/* Strip leading emoji from title (backend includes them) */}
                            ...
                            <div className="ios-notif-actions">
                              {isViewable(n) && (
                                <button
                                  className="ios-notif-btn ios-notif-btn--primary"
                              ...
                              {!n.read && (
                                <button
                                  className="ios-notif-btn ios-notif-btn--ghost"
